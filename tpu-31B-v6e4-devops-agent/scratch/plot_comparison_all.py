...
paths = {
    ...
    "31B (v6e-4)": "grid_benchmark_results.csv",
...
with open(report_path, "w") as f:
    ...
    f.write("This report compares the request throughput (req/s) across different model configurations and hardware node sizes (4-chip v6e-4 vs 1-chip v6e-1).\n\n")
    ...
    f.write("1. **Node Size Scaling Advantage (v6e-4 vs v6e-1):** The 4-chip v6e-4 node provides a massive processing advantage compared to the single-chip v6e-1. For example, the 4B model at Concurrency 64 scales from `55.33 req/s` on v6e-1 to `88.11 req/s` on v6e-4.\n")
    f.write("2. **Large Model Servability:** Large models like 26B and 31B require the distributed memory and sharding topology of the 4-chip v6e-4 node (128GB HBM total) to run context sweeps efficiently without severe preemption or OOM errors.\n")
