...
mcp = FastMCP("tpu-31B-v6e4-devops-agent")
...
async def query_queued_gemma4_with_stats(prompt: str) -> str:
    ...
    try:
        ...
        async for chunk in stream:
            ...
            content = getattr(chunk.choices[0].delta, "content", None) or getattr(chunk.choices[0].delta, "reasoning", None)
            if content:
...
async def find_gpu(
    ...
    if gpu_vms:
        ...
        report.append("| :--- | :--- | :--- | :--- | :--- |")
        for vm in gpu_vms:
            report.append(f"| **{vm['name']}** | `{vm['zone']}` | `{vm['machine_type']}` | `{vm['status']}` | {vm['accelerators']} |")
    else:
