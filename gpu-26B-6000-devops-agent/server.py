...
MODEL_NAME = os.getenv("MODEL_NAME", "google/gemma-4-26B-it")
...
def discover_vllm_url(service_name: str = "gpu-26b-6000-devops-agent") -> Optional[str]:
...
# Required: NVIDIA RTX PRO 6000 GPU
# Required: GCS FUSE mount

service: gpu-26b-6000-devops-agent
...
# gcloud run deploy gpu-26b-6000-devops-agent --no-cpu-throttling --allow-unauthenticated --concurrency=4 \\
...
@mcp.tool()
def get_vllm_endpoint(service_name: str = "gpu-26b-6000-devops-agent") -> Optional[str]:
    """
    Returns the current active vLLM endpoint URL.

    Args:
        service_name: The Cloud Run service name to describe (defaults to 'gpu-26b-6000-devops-agent').
    """
    # If it's the default service, use our cached discovery logic
    if service_name == "gpu-26b-6000-devops-agent":
        return get_vllm_url()
...
def get_vllm_deployment_config(
    service_name: str = "gpu-26b-6000-devops-agent",
    bucket_name: str = BUCKET_NAME,
    model_path: str = "gemma-4-26B-A4B-it",
    allow_unauthenticated: bool = False,
    min_instances: int = 1,
    gpu_memory_utilization: float = 0.95,
) -> str:
    """
    Generates the gcloud command to deploy vLLM to Cloud Run with GCS FUSE and NVIDIA RTX PRO 6000 GPU.

    Args:
        service_name: The name for the Cloud Run service.
        bucket_name: The GCS bucket containing the model weights.
        model_path: The sub-path inside the bucket (e.g., 'gemma-4-26B-it') or Hugging Face repo ID.
...
@mcp.tool()
def destroy_vllm(service_name: str = "gpu-26b-6000-devops-agent") -> str:
    """
    Deletes the Cloud Run vLLM service.
...
@mcp.tool()
def status_vllm(service_name: str = "gpu-26b-6000-devops-agent") -> str:
    """
    Retrieves the status of the Cloud Run service.
...
@mcp.tool()
def update_vllm_scaling(min_instances: int, max_instances: int, service_name: str = "gpu-26b-6000-devops-agent") -> str:
    """
...
@mcp.tool()
def get_vertex_ai_model_copy_instructions(model_name: str = "gemma-4-26B-it") -> str:
    """
...
async def get_huggingfacehub_download_path(
    repo_id: str = "google/gemma-4-26B-it",
) -> str:
    """
...
@mcp.tool()
async def get_system_status(service_name: str = "gpu-26b-6000-devops-agent") -> str:
    """
...
@mcp.tool()
async def get_endpoint(service_name: str = "gpu-26b-6000-devops-agent") -> str:
    """
...
@mcp.tool()
async def analyze_gpu_logs(limit: int = 15, service_name: str = "gpu-26b-6000-devops-agent") -> str:
    """
...
async def get_help() -> str:
    ...
    return (
        ...
        "The server is running in **CLOUD RUN** mode targeting NVIDIA RTX PRO 6000 GPU in region `us-central1`.\n\n"
        "---\n\n"
        "### 🧰 Available MCP Tools\n\n"
