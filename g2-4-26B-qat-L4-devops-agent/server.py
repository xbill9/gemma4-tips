...
VLLM_BASE_URL = "http://35.243.250.174:8080"
...
docker run -d --name vllm-server \\
  ...
  --model google/gemma-4-26B-A4B-it-qat-w4a16-ct \\
...
def get_vllm_endpoint(service_name: str = DEFAULT_SERVICE_NAME) -> Optional[str]:
    ...
    Args:
        service_name: The service name or instance Name tag to describe (defaults to 'gpu-26b-qat-l4-devops-agent').
    """
...
def get_vllm_deployment_config(
    ...
    model_path: str = "google/gemma-4-26B-A4B-it-qat-w4a16-ct",
    zone: str = ZONE,
...
"""
    ...
    return (
        f"### 🚀 GCP GCE g2-standard-4 (NVIDIA L4) Instance vLLM Deployment Config\n\n"
...
def get_vllm_gpu_deployment_config(
    cluster_name: str = "gpu-cluster", model_name: str = "google/gemma-4-26B-A4B-it-qat-w4a16-ct"
) -> str:
    """
    ...
    Args:
        ...
        model_name: The model identifier (e.g., 'google/gemma-4-26B-A4B-it-qat-w4a16-ct').
    """
    manifest = f"""
### 🌀 vLLM on GPU (GKE Deployment)

To deploy vLLM on GPUs, use the following GKE manifest. This configuration targets a single **NVIDIA L4 GPU** using a **g2-standard-4** node.
