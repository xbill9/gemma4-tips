...
def discover_vllm_url(service_name="gpu-26b-qat-l4-devops-agent"):
...
async def run_sweep():
    ...
    model_name = "/mnt/models/gemma-4-26B-A4B-it-qat-w4a16-ct"
    try:
        headers = {}
        if token:
            headers["Authorization"] = f"Bearer {token}"
    ...
    with open(md_file, "w") as f:
        ...
        f.write(f"Model: `{model_name}` (NVIDIA L4 GPU GCE VM)\n\n")
