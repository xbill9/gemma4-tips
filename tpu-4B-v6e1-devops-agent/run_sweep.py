...
for c in concurrencies:
    for ctx in contexts:
        ...
        try:
            ...
            cmd = f'gcloud compute tpus tpu-vm ssh vllm-gemma4-qr-node --zone=europe-west4-a --project=comglitn --command="sudo docker exec vllm-gemma4 vllm bench serve --host localhost --port 8000 --model google/gemma-4-E4B-it --dataset-name random --num-prompts {num_prompts} --random-input-len {ctx} --random-output-len 100 --max-concurrency {c}"'
