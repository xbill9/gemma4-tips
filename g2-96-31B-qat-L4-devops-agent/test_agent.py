...
class TestDevOpsAgent(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_deploy_vllm(self, mock_get_secret, mock_run_gcloud):
        ...
        self.assertIn(
            "Successfully requested GCP GCE g2-standard-96 Instance deployment for service 'test-service'", result
        )
        mock_run_gcloud.assert_called()
    ...
    async def test_status_vllm(self, mock_run_gcloud):
        ...
        mock_describe_json = (
            '{"id": "123456789", "status": "RUNNING", "machineType": "zones/us-east4-a/machineTypes/g2-standard-96", '
        ...
        self.assertIn("g2-standard-96", result)
    ...
    def test_get_vllm_deployment_config_gce(self):
        ...
        self.assertIn("GCP GCE g2-standard-96 (8x NVIDIA L4) Instance vLLM Deployment Config", result)
        self.assertIn("gcloud compute instances create", result)
        self.assertIn("--machine-type=g2-standard-96", result)
        self.assertIn("--accelerator=type=nvidia-l4,count=8", result)
