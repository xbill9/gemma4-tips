...
def generate_plots():
    ...
    for c in concurrencies:
        ...
        ax1.plot(sub_df['max_tokens'], sub_df['tokens_sec'], 
                 marker='o', markersize=6, linewidth=2.5, 
                 color=colors.get(c, '#ffffff'), label=f'Concurrency: {c}')
    ...
    for c in concurrencies:
        ...
        ax2.plot(sub_df['max_tokens'], sub_df['avg_latency'], 
                 marker='o', markersize=6, linewidth=2.5, 
                 color=colors.get(c, '#ffffff'), label=f'C={c} Avg')
        # P95 Latency: Dashed line with x's
        ax2.plot(sub_df['max_tokens'], sub_df['p95_latency'], 
                 marker='x', markersize=6, linewidth=1.5, linestyle='--',
                 color=colors.get(c, '#ffffff'), alpha=0.8, label=f'C={c} P95')
    ...
    for c in concurrencies:
        sub_df = df[df['concurrency'] == c]
        ax3.plot(sub_df['max_tokens'], sub_df['throughput_req_sec'], 
                 marker='o', markersize=6, linewidth=2.5, 
                 color=colors.get(c, '#ffffff'), label=f'Concurrency: {c}')
