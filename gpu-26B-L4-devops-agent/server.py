...
MODEL_NAME = os.getenv("MODEL_NAME", "nvidia/gemma-4-26B-A4B-NVFP4")
...
DEFAULT_SERVICE_NAME = "gpu-26b-l4-devops-agent"
...
#   --max-instances=1 --args=--model=/mnt/models/gemma-4-26B-it,--dtype=float16,--quantization=fp8,--max-model-len=16384,--disable-chunked-mm-input,--gpu-memory-utilization=0.95,--kv-cache-dtype=fp8,--tensor-parallel-size=1,--max-num-seqs=4,--enable-chunked-prefill,--max-num-batched-tokens=4096,--enable-auto-tool-choice,--tool-call-parser=gemma4,--reasoning-parser=gemma4,--async-scheduling,--limit-mm-per-prompt={{}},--host=0.0.0.0,--port=8000
volumes:
  - name: model-volume
    cloudStorage:
      bucket: {BUCKET_NAME}
...
def get_vllm_endpoint(service_name: str = DEFAULT_SERVICE_NAME) -> Optional[str]:
    ...
    Args:
        service_name: The Cloud Run service name to describe (defaults to 'gpu-26b-l4-devops-agent').
    """
...
def get_vllm_deployment_config(
    ...
    model_path: str = "nvidia/gemma-4-26B-A4B-NVFP4",
    ...
    Args:
        ...
        model_path: The sub-path inside the bucket (e.g., 'nvidia/gemma-4-26B-A4B-NVFP4') or Hugging Face repo ID.
    ...
    quantization = "fp8"
    if "nvfp4" in model_path.lower() or "fp4" in model_path.lower():
        quantization = "nvfp4"
...
@mcp.tool()
def get_vertex_ai_model_copy_instructions(model_name: str = "gemma-4-26B-A4B-it") -> str:
    """
...
def get_huggingface_model_copy_instructions(
    ...
    Args:
        repo_id: The Hugging Face repo ID (e.g., 'google/gemma-4-26B-it').
