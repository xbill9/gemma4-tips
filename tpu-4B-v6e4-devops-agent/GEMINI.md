...
*   **Hardware:** Cloud TPU v6e (Trillium) with topology `2x2` (4 chips).
...
*   **`create_tpu_queued_resource`**: Creates a TPU Queued Resource (Flex-start) with the specified configuration and zone.
...
*   **`get_zones_with_available_quota`**: Retrieves a list of GCP zones that have a non-zero quota for a specific metric.
*   **`find_tpu`**: Finds a zone with available quota and attempts to create the TPU queued resource in that zone until successful.
*   **`find_gpu`**: Finds available GPU resources (GCE VMs, Cloud Run services, and zones with available GPU quota) in the GCP project.

### Observability & Performance
*   **`get_system_status`**: Provides a high-level dashboard of system status.
...
*   **`get_active_models`**: Gets the active resource usage via ollama ps.
*   **`get_model_show_details`**: Gets deep model parameters via ollama show.
