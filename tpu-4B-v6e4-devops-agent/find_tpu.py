...
def find_tpu_queued_resources():
    ...
    stdout = run_gcloud([
        "alpha", "compute", "tpus", "queued-resources", "list",
        f"--project={PROJECT_ID}",
        "--zone=-",
...
def main():
    ...
    if tpu_vms:
        ...
        report.append("| :--- | :--- | :--- | :--- | :--- |")
        for tpu in tpu_vms:
            report.append(f"| **{tpu['name']}** | `{tpu['zone']}` | `{tpu['accelerator_type']}` | `{tpu['topology']}` | `{tpu['status']}` |")
    else:
    ...
    if tpu_qrs:
        ...
        report.append("| :--- | :--- | :--- | :--- | :--- |")
        for qr in tpu_qrs:
            report.append(f"| **{qr['name']}** | `{qr['zone']}` | `{qr['accelerator_type']}` | `{qr['topology']}` | `{qr['state']}` |")
    else:
