...
def heatmap(rows, out):
    ...
    ax.set_title("Gemma 4 E4B on TPU v5e-1 — total throughput (tokens/s)", color=INK, fontsize=13, pad=36, loc="left")
    ax.text(
        0,
        1.018,
...
def lines(rows, out):
    ...
    fig.suptitle(
        "Gemma 4 E4B on TPU v5e-1 — concurrency x context sweep", color=INK, fontsize=13, x=0.007, ha="left", y=0.99
    )
