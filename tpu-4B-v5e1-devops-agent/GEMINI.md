...
Now, every time you run `gemini`, it will be powered by your private TPU v5e cluster.
...
*   **`get_zones_with_available_quota`**: Gets GCP zones that have available TPU quota for TPU v5e.
*   **`find_tpu`**: Scans multiple zones sequentially to find and provision a TPU v5e resource.
*   **`find_gpu`**: Scans the GCP project for available GPU resources (GCE VMs, Cloud Run services, and zones with quota).
*   **`get_vllm_deployment_config`**: Generates the `gcloud` command for a single-host TPU v5e vLLM deployment.
