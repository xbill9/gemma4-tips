...
mcp = FastMCP("tpu-26B-v6e4-devops-agent")

# --- Configuration ---
PROJECT_ID = os.getenv("GOOGLE_CLOUD_PROJECT", "aisprint-491218")
ZONE = "southamerica-west1-a"
REGION = "southamerica-west1"
MODEL_NAME = os.getenv("MODEL_NAME", "hugg1ngfac3/gemma-4-26B-A4B-it-FP8")
if MODEL_NAME == "google/gemma-4-26B-A4B-it":
    MODEL_NAME = "hugg1ngfac3/gemma-4-26B-A4B-it-FP8"
...
@mcp.tool()
async def manage_queued_resource(resource_id: str = "vllm-gemma4-qr", zone: str = ZONE) -> str:
...
@mcp.tool()
async def create_tpu_queued_resource(resource_id: str = "vllm-gemma4-qr", zone: str = ZONE) -> str:
...
async def manage_vllm_docker(resource_id: str = "vllm-gemma4-qr", action: str = "start") -> str:
    ...
    docker_run_cmd = (
        ...
        f"-e MOE_REQUANTIZE_WEIGHT_DTYPE=float8_e4m3fn -e JAX_TPU_MEM_FRACTION=0.95 "
        f"-e VLLM_TPU_BUCKET_PADDING_GAP=512 -e VLLM_XLA_CACHE_PATH=/dev/shm/vllm_cache "
        f"{docker_image} vllm serve {MODEL_NAME} "
        f"--tensor-parallel-size {TENSOR_PARALLEL_SIZE} --dtype bfloat16 --kv-cache-dtype fp8 --block-size 32 "
        f"--disable_chunked_mm_input --max-model-len 16384 --gpu-memory-utilization 0.80 "
        f"--max-num-batched_tokens 4096 --enable-auto-tool-choice --tool-call-parser gemma4 --reasoning-parser gemma4 "
...
async def estimate_deployment_cost(
    hours: float = 1.0, tpu_type: str = "v6e", topology: str = "2x2", is_flex: bool = True
) -> str:
...
async def run_vllm_benchmark(
    resource_id: str = "vllm-gemma4-qr",
    backend: str = "vllm",
    model: str = "hugg1ngfac3/gemma-4-26B-A4B-it-FP8",
