...
def main():
    ...
    report.append("> [!NOTE]")
    report.append(
        "> This report summarizes the performance characteristics of `google/gemma-4-31B-it` served via vLLM on a Cloud TPU v6e-8 (8 chips) cluster in `southamerica-west1-a`."
    )
    report.append(
        "> The benchmark sweeps across concurrent users (1 to 2048) and context window lengths (8 to 16K tokens).\n"
    )
