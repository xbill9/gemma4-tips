...
REGION = os.getenv("GOOGLE_CLOUD_REGION", "southamerica-west1")
...
async def discover_vllm_url() -> Optional[str]:
    ...
    env_url = os.getenv("ACTIVE_VLLM_URL")
    if env_url:
        logger.info(f"📡 Using active vLLM URL from environment override: {env_url}")
        return env_url
    ...
    if rc != 0 or not stdout:
        fallback_url = "http://34.176.180.155:8000"
        logger.warning(f"⚠️ gcloud discovery failed. Falling back to active endpoint: {fallback_url}")
        return fallback_url
