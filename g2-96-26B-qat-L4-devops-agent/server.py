...
# Required Instance: g2-standard-96 (8x NVIDIA L4 GPU, 192GB VRAM)
# Image: Deep Learning VM with CUDA 12.1+

InstanceType: g2-standard-96
Accelerators: 8x nvidia-l4
Ports:
  - Container Port: 8080
  - Host Port: 8080
...
def get_vllm_deployment_config(
    ...
    """
    Generates the gcloud compute command and startup script to deploy vLLM to a GCP GCE g2-standard-96 instance (8x NVIDIA L4).
...
"""
    ...
    gcloud_cmd = (
        ...
        f"  --project={PROJECT_ID} \\\n"
        f"  --zone={zone} \\\n"
        f"  --machine-type=g2-standard-96 \\\n"
        f"  --accelerator=type=nvidia-l4,count=8 \\\n"
    ...
    return (
        f"### 🚀 GCP GCE g2-standard-96 (8x NVIDIA L4) Instance vLLM Deployment Config\n\n"
...
async def deploy_vllm(
    ...
    """
    Deploys vLLM to GCP GCE g2-standard-96 (8x NVIDIA L4) VM instance.
...
"""
    ...
    try:
        ...
        return (
            f"🚀 Successfully requested GCP GCE g2-standard-96 Instance deployment for service '{service_name}' in zone '{zone}'.\n"
            f"Machine Type: `g2-standard-96`\n"
            f"Accelerator: `8x NVIDIA L4 (192GB total VRAM)`\n"
...
def get_vllm_gpu_deployment_config(
    ...
    """
    Generates a GKE manifest and setup instructions for deploying vLLM on GPU (8x NVIDIA L4).
...
To deploy vLLM on GPUs, use the following GKE manifest. This configuration targets **8x NVIDIA L4 GPUs** using a **g2-standard-96** node.
...
async def get_help() -> str:
    ...
    return (
        ...
        "The server is running in **GCP GCE VM** mode targeting a `g2-standard-96` host VM with 8x NVIDIA L4 GPUs sharded via Tensor Parallelism (TP=8).\n\n"
        "### 🧰 Available MCP Tools\n\n"
        ...
        "- **`deploy_vllm`**: Deploys vLLM to GCP GCE g2-standard-96 (8x NVIDIA L4) VM instance.\n"
