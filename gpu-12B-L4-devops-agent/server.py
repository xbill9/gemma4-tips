...
DEFAULT_SERVICE_NAME = "gpu-12b-l4-devops-agent"
...
def get_vllm_deployment_config(
    ...
    command = [
        ...
        "--startup-probe=timeoutSeconds=60,periodSeconds=60,failureThreshold=25,initialDelaySeconds=180,httpGet.port=8000,httpGet.path=/health",
    ...
    if is_hf:
        ...
        shell_script = (
            f"mkdir -p /tmp/model && "
            f"echo '⌛ Copying model weights from GCS FUSE to local tmpfs...' && "
            f"cp -r /mnt/models/{model_path}/* /tmp/model/ && "
            f"echo '✅ Copy complete. Starting vLLM...' && "
        ...
        command.append(f"--args='^|^{shell_script}'")
...
async def deploy_vllm(
    ...
    if is_hf:
        ...
        cmd.append(f"--args=^|^{shell_script}")
