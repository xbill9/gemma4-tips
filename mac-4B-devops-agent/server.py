...
async def manage_docker(action: str = "status") -> str:
    """Manages the local vLLM/Ollama Docker container or native service (actions: start, stop, restart, status, log, rm)."""
    ollama_model = get_current_model_name()
    if not await is_docker_available():
        logger.info(f"Local deployment mode: Managing native Ollama with action '{action}'")
        commands = {
            "start": f"brew services start ollama && sleep 2 && ollama pull {ollama_model}",
            "stop": "brew services stop ollama",
            "restart": "brew services restart ollama",
            "status": "brew services list | grep ollama",
            "log": "tail -n 100 /opt/homebrew/var/log/ollama.log",
            "rm": "echo 'rm not supported/necessary for native Ollama service'",
        }
        ...
        try:
            ...
            if rc != 0:
                return f"❌ Local native Ollama {action} failed: {stderr.decode().strip() or stdout.decode().strip()}"
            return f"✅ Local native Ollama {action} command executed:\n{stdout.decode().strip()}"
        except Exception as e:
            return f"❌ Local native Ollama {action} execution error: {e}"
...
async def get_system_status() -> str:
    ...
    if await is_docker_available():
        ...
        docker_label = "Docker Container Status"
    else:
        docker_status = "🔴 Stopped"
        try:
            rc, out, _ = await run_command(["brew", "services", "list"])
            ...
            docker_status = f"🔴 Error checking native service: {e}"
        docker_label = "Native Ollama Service Status"
    ...
    if "🟢" in health:
        ...
        if await is_docker_available():
            ...
            next_step = "Call `manage_docker` with action='start' to start the native Ollama service."

    return (
        f"### 🌀 Local System Status\n"
        f"- **vLLM Health:** {health}\n"
        f"- **{docker_label}:** {docker_status}\n"
...
async def get_docker_logs(tail: Optional[int] = None) -> str:
    """Retrieves logs from the local vLLM/Ollama Docker container or native service."""
    if not await is_docker_available():
        log_cmd = "tail -n 100 /opt/homebrew/var/log/ollama.log"
        if tail:
            log_cmd = f"tail -n {tail} /opt/homebrew/var/log/ollama.log"
        rc, out, err = await run_command(shlex.split(log_cmd))
        if rc != 0:
            return f"⚠️ Failed to get local native Ollama logs.\nError: {err}"
        return f"✅ Local native Ollama logs:\n{out}"
