...
class TestDevOpsAgent(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_deploy_vllm_hf(self, mock_run):
        ...
        self.assertIn("--command=bash", cmd)
        self.assertIn(
            "--args=^;^-c;vllm serve google/gemma-4-E4B-it --served-model-name test-service --enable-log-requests --enable-chunked-prefill --enable-prefix-caching --generation-config auto --enable-auto-tool-choice --tool-call-parser gemma4 --reasoning-parser gemma4 --dtype bfloat16 --quantization fp8 --kv-cache-dtype fp8 --max-num-seqs 8 --gpu-memory-utilization 0.95 --tensor-parallel-size 1 --load-format runai_streamer --port 8080 --host 0.0.0.0 --max-model-len 32767",
            cmd,
        )
