...
# Required Instance: g2-standard-48 (4x NVIDIA L4 GPU, 96GB VRAM)
# Image: Deep Learning VM with CUDA 12.1+

InstanceType: g2-standard-48
Accelerators: 4x nvidia-l4
Ports:
  - Container Port: 8080
  - Host Port: 8080
...
def get_vllm_deployment_config(
    ...
    """
    Generates the gcloud compute command and startup script to deploy vLLM to a GCP GCE g2-standard-48 instance (4x NVIDIA L4).
...
"""
    ...
    gcloud_cmd = (
        ...
        f"  --project={PROJECT_ID} \\\n"
        f"  --zone={zone} \\\n"
        f"  --machine-type=g2-standard-48 \\\n"
        f"  --accelerator=type=nvidia-l4,count=4 \\\n"
    ...
    return (
        f"### 🚀 GCP GCE g2-standard-48 (4x NVIDIA L4) Instance vLLM Deployment Config\n\n"
...
async def deploy_vllm(
    ...
    """
    Deploys vLLM to GCP GCE g2-standard-48 (4x NVIDIA L4) VM instance.
...
"""
    ...
    try:
        ...
        return (
            f"🚀 Successfully requested GCP GCE g2-standard-48 Instance deployment for service '{service_name}' in zone '{zone}'.\n"
            f"Machine Type: `g2-standard-48`\n"
            f"Accelerator: `4x NVIDIA L4 (96GB total VRAM)`\n"
...
def get_vllm_gpu_deployment_config(
    ...
    """
    Generates a GKE manifest and setup instructions for deploying vLLM on GPU (4x NVIDIA L4).
...
To deploy vLLM on GPUs, use the following GKE manifest. This configuration targets **4x NVIDIA L4 GPUs** using a **g2-standard-48** node.
...
async def find_quota(resource_type: str = "NVIDIA_L4") -> str:
    ...
    for region in regions:
        ...
        if code == 0:
            ...
            for q in quotas:
                metric = q.get("metric", "")
                if resource_type.upper() in metric.upper() or (
                    "GPU" in metric.upper() and resource_type.upper() == "NVIDIA_L4"
                ):
    ...
    return (
        f"### ✅ Available Quota for `{resource_type}`\n\n"
        + "\n".join(results)
...
async def find_tpu(accelerator_type: str = "v6e-4") -> str:
    ...
    search_regions = (
        tpu_regions if tpu_regions else ["us-central1", "us-east1", "us-east5", "europe-west4", "asia-southeast1"]
    )
...
async def get_help() -> str:
    ...
    return (
        ...
        "The server is running in **GCP GCE VM** mode targeting a `g2-standard-48` host VM with 4x NVIDIA L4 GPUs sharded via Tensor Parallelism.\n\n"
        "### 🧰 Available MCP Tools\n\n"
        ...
        "- **`deploy_vllm`**: Deploys vLLM to GCP GCE g2-standard-48 (4x NVIDIA L4) VM instance.\n"
