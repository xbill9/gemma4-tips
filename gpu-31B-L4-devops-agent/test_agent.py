...
class TestDevOpsAgent(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_deploy_vllm_hf(self, mock_run):
        ...
        args, kwargs = mock_run.call_args
        cmd = args[0]
        from server import HF_SECRET_ID

        self.assertIn(f"--set-secrets=HF_TOKEN={HF_SECRET_ID}:latest", cmd)
        self.assertNotIn(
            "--add-volume=name=model-volume,type=cloud-storage,bucket=test-bucket,readonly=true,mount-options=uid=1001;gid=1001",
            cmd,
        )
    ...
    def test_get_vertex_ai_model_copy_instructions(self):
        ...
        instructions = get_vertex_ai_model_copy_instructions("gemma-4-31B-it")
        self.assertIn("gemma-4-31B-it", instructions)
    ...
    def test_list_bucket_models_mock(self, mock_storage_client):
        ...
        mock_blob = MagicMock()
        mock_blob.name = "gemma-4-31B-it/config.json"
        ...
        self.assertIn("gemma-4-31B-it/config.json", result)
