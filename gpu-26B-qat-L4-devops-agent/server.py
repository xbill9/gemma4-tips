...
MODEL_NAME = os.getenv("MODEL_NAME", "google/gemma-4-26B-A4B-it-qat-w4a16-ct")
...
DEFAULT_SERVICE_NAME = "gpu-26b-qat-l4-devops-agent"
...
#   --max-instances=1 --args=--model=/mnt/models/gemma-4-26B-A4B-it-qat-w4a16-ct,--dtype=bfloat16,--max-model-len=16384,--disable-chunked-mm-input,--gpu-memory-utilization=0.90,--kv-cache-dtype=fp8,--tensor-parallel-size=1,--max-num-seqs=8,--enable-chunked-prefill,--max-num-batched-tokens=4096,--enable-auto-tool-choice,--tool-call-parser=gemma4,--reasoning-parser=gemma4,--async-scheduling,--limit-mm-per-prompt={{}},--host=0.0.0.0,--port=8080
volumes:
  - name: model-volume
    cloudStorage:
      bucket: {BUCKET_NAME}
...
def get_vllm_endpoint(service_name: str = DEFAULT_SERVICE_NAME) -> Optional[str]:
    ...
    Args:
        service_name: The Cloud Run service name to describe (defaults to 'gpu-26b-qat-l4-devops-agent').
    """
...
def get_vllm_deployment_config(
    ...
    model_path: str = "gemma-4-26B-A4B-it-qat-w4a16-ct",
    ...
    Args:
        ...
        model_path: The sub-path inside the bucket (e.g., 'gemma-4-26B-A4B-it-qat-w4a16-ct') or Hugging Face repo ID.
    ...
    command = [
        ...
        "--startup-probe=timeoutSeconds=60,periodSeconds=60,failureThreshold=25,initialDelaySeconds=240,httpGet.port=8080,httpGet.path=/health",
    ...
    if is_hf:
        ...
        command.append(
            f"--args=--model=/mnt/models/{model_path}{quant_arg},--dtype=bfloat16,--max-model-len=16384,--disable-chunked-mm-input,--gpu-memory-utilization={gpu_memory_utilization},--kv-cache-dtype=fp8,--tensor-parallel-size=1,--max-num-seqs=8,--enable-chunked-prefill,--max-num-batched-tokens=4096,--enable-auto-tool-choice,--tool-call-parser=gemma4,--reasoning-parser=gemma4,--async-scheduling,--limit-mm-per-prompt={{}},--host=0.0.0.0,--port=8080"
        )
...
async def deploy_vllm(
    ...
    if is_hf:
        ...
        cmd.append(
            f"--args=--model=/mnt/models/{model_path}{quant_arg},--dtype=bfloat16,--max-model-len=16384,--disable-chunked-mm-input,--gpu-memory-utilization=0.90,--kv-cache-dtype=fp8,--tensor-parallel-size=1,--max-num-seqs=8,--enable-chunked-prefill,--max-num-batched-tokens=4096,--enable-auto-tool-choice,--tool-call-parser=gemma4,--reasoning-parser=gemma4,--async-scheduling,--limit-mm-per-prompt={{}},--safetensors-load-strategy=prefetch,--host=0.0.0.0,--port=8080"
        )
...
@mcp.tool()
def get_vertex_ai_model_copy_instructions(model_name: str = "gemma-4-26B-A4B-it-qat-w4a16-ct") -> str:
    """
...
async def get_huggingfacehub_download_path(
    repo_id: str = "google/gemma-4-26B-A4B-it-qat-w4a16-ct",
) -> str:
    """
...
def get_huggingface_model_copy_instructions(
    ...
    Args:
        repo_id: The Hugging Face repo ID (e.g., 'google/gemma-4-26B-A4B-it-qat-w4a16-ct').
