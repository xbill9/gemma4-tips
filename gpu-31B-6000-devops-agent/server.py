...
def discover_vllm_url(service_name: str = "gpu-31b-6000-devops-agent") -> Optional[str]:
...
service: gpu-31b-6000-devops-agent
...
# gcloud run deploy gpu-31b-6000-devops-agent --no-cpu-throttling --allow-unauthenticated --concurrency=4 \\
...
@mcp.tool()
def get_vllm_endpoint(service_name: str = "gpu-31b-6000-devops-agent") -> Optional[str]:
    """
    Returns the current active vLLM endpoint URL.

    Args:
        service_name: The Cloud Run service name to describe (defaults to 'gpu-31b-6000-devops-agent').
    """
    # If it's the default service, use our cached discovery logic
    if service_name == "gpu-31b-6000-devops-agent":
        return get_vllm_url()
...
def get_vllm_deployment_config(
    service_name: str = "gpu-31b-6000-devops-agent",
    bucket_name: str = BUCKET_NAME,
    model_path: str = "gemma-4-31B-it",
...
@mcp.tool()
def destroy_vllm(service_name: str = "gpu-31b-6000-devops-agent") -> str:
    """
...
@mcp.tool()
def status_vllm(service_name: str = "gpu-31b-6000-devops-agent") -> str:
    """
...
@mcp.tool()
def update_vllm_scaling(min_instances: int, max_instances: int, service_name: str = "gpu-31b-6000-devops-agent") -> str:
    """
...
@mcp.tool()
def get_vertex_ai_model_copy_instructions(model_name: str = "gemma-4-31B-it") -> str:
    """
...
async def get_huggingfacehub_download_path(
    repo_id: str = "google/gemma-4-31B-it",
) -> str:
    """
...
@mcp.tool()
async def get_system_status(service_name: str = "gpu-31b-6000-devops-agent") -> str:
    """
...
@mcp.tool()
async def get_endpoint(service_name: str = "gpu-31b-6000-devops-agent") -> str:
    """
...
@mcp.tool()
async def analyze_gpu_logs(limit: int = 15, service_name: str = "gpu-31b-6000-devops-agent") -> str:
    """
