...
from server import get_vllm_url, get_auth_token, get_vllm_client, get_active_model_name

async def send_request(client: httpx.AsyncClient, base_url: str, model: str, prompt: str, max_tokens: int, headers: dict) -> dict:
    payload = {
        "model": model,
        "prompt": prompt,
...
async def run_sweep():
    ...
    for ctx_size in context_sizes:
        ...
        for concurrency in concurrencies:
            ...
            # To avoid overloading or taking too long, cap total prompts at 256
            if num_prompts > 256:
                num_prompts = 256
            ...
            print(f"✅ Success Rate: {success_rate:.1%}, Throughput: {throughput:.2f} req/s, Tokens: {tokens_per_sec:.2f} tok/s, Avg Latency: {avg_lat:.2f}s")
