...
class TestDevOpsAgent(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_deploy_vllm(self, mock_run):
        ...
        self.assertIn(
            "--args=^;^-c;vllm serve /mnt/models/test-model --served-model-name test-service --enable-log-requests --enable-chunked-prefill --enable-prefix-caching --generation-config auto --enable-auto-tool-choice --tool-call-parser gemma4 --reasoning-parser gemma4 --dtype bfloat16 --kv-cache-dtype fp8 --max-num-seqs 8 --gpu-memory-utilization 0.95 --tensor-parallel-size 1 --load-format runai_streamer --port 8080 --host 0.0.0.0 --max-model-len 131072",
            cmd,
        )
    ...
    async def test_deploy_vllm_hf(self, mock_run):
        ...
        self.assertIn("--image=vllm/vllm-openai:latest", cmd)
        self.assertIn(
            "--args=^;^-c;vllm serve google/gemma-4-12B-it --served-model-name test-service --enable-log-requests --enable-chunked-prefill --enable-prefix-caching --generation-config auto --enable-auto-tool-choice --tool-call-parser gemma4 --reasoning-parser gemma4 --dtype bfloat16 --kv-cache-dtype fp8 --max-num-seqs 8 --gpu-memory-utilization 0.95 --tensor-parallel-size 1 --load-format runai_streamer --port 8080 --host 0.0.0.0 --max-model-len 131072",
            cmd,
        )
    ...
    def test_get_vertex_ai_model_copy_instructions(self):
        ...
        instructions = get_vertex_ai_model_copy_instructions("gemma-4-12B-it")
        self.assertIn("gemma-4-12B-it", instructions)
    ...
    def test_list_bucket_models_mock(self, mock_storage_client):
        ...
        mock_blob = MagicMock()
        mock_blob.name = "gemma-4-12B-it/config.json"
        ...
        self.assertIn("gemma-4-12B-it/config.json", result)
    ...
    async def test_get_help(self):
        ...
        self.assertIn("NVIDIA RTX PRO 6000", result)
        self.assertIn("us-central1", result)
