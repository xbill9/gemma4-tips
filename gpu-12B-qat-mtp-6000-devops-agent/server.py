...
def get_vllm_endpoint(service_name: str = DEFAULT_SERVICE_NAME) -> Optional[str]:
    ...
    Args:
        service_name: The Cloud Run service name to describe (defaults to 'gpu-12b-l4-devops-agent').
    """
...
def get_vllm_deployment_config(
    ...
    # Detect quantization format if not explicitly provided
    if quantization is None:
        if "qat" in model_path.lower() or "ct" in model_path.lower() or "compressed-tensors" in model_path.lower():
    ...
    command.append(
        f'--set-env-vars="MODEL_NAME={model_name_val},GOOGLE_CLOUD_PROJECT={PROJECT_ID},GOOGLE_CLOUD_REGION={LOCATION},{hf_env}"'
    )
    ...
    container_args = [arg for arg in container_args if arg]
...
async def get_help() -> str:
    ...
    return (
        ...
        "- **`check_gpu_quotas`**: Checks RTX PRO 6000 and other GPU quotas for a region.\n\n"
        "#### 📊 Model Management\n"
