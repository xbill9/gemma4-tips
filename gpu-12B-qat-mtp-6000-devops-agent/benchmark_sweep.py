...
def discover_vllm_url(service_name="gpu-12b-6000-devops-agent"):
...
async def run_sweep():
    ...
    async with httpx.AsyncClient(limits=httpx.Limits(max_connections=10)) as client:
        await send_req(client, sem, prompts_cache[4096])
    ...
    with open(md_file, "w") as f:
        ...
        f.write(f"Model: `{model_name}` (NVIDIA RTX PRO 6000 GPU Cloud Run)\n\n")
    ...
    try:
        ...
        plot_sizes = [4096, 16384, 65536, 131071]
        markers = ["o", "s", "d", "^"]
